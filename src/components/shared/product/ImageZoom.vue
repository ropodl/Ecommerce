  ...
  >
    ...
    >
      <picture :class="{ 'vh--none': zoomed }">
        ...
        <img
          ...
          :class="imgClass"
          :alt="alt"
          @load="$emit('regularLoaded'), (showSlot = false)"
        />
      </picture>
      <picture v-if="zoomed">
        <template v-for="breakpoint in breakpoints" :key="breakpoint.width">
    ...
    <transition name="VueHoverfade">
      ...
      <div
        ...
        v-else-if="loading"
      >
        <div class="vh--loading" v-html="'&#9696;'"></div>
      </div>
    </transition>
  </div>
</template>
...
export default {
  ...
  methods: {
    async touchLogic() {
      ...
      this.$refs["vue-hover-zs"].addEventListener("touchmove", (e) => {
        if (this.zoomed) {
          ...
          if (
            touchmovement.pageY - sy <=
            this.origY - this.options.zoomAmount * this.origY
          ) {
    ...
    isZoom(type, action) {
      if (
        (action == "hover" && !this.clickZoom && !this.touch) ||
        (action == "click" && (this.clickZoom || this.touch)) ||
        typeof type === "object"
      ) {
        //set zoomed to false
        this.zoomed = false;
    ...
    mobilePos() {
      ...
      if (
        this.touchZoomPos[0] > 1 ||
        this.touchZoomPos[0] < 0 ||
        this.touchZoomPos[1] > 1 ||
        this.touchZoomPos[1] < 0
      ) {
        X = 0;
        Y = 0;
      }
    ...
    mousePos(e) {
      ...
      this.offsetLeft =
        window.pageXOffset +
        this.$refs["vue-hover-zs"].getBoundingClientRect().left;
      this.offsetTop =
        window.pageYOffset +
        this.$refs["vue-hover-zs"].getBoundingClientRect().top;
